...
extern void draw_square(image picture, int xmin, int ymin, int xmax, int ymax, unsigned char* color){
   ...
   i=xmin;
   j=ymin;
   image_move_to(picture,&high);
   do{
      image_write_pixel(picture,i,j,color);  /*on fait le trait du point en haut à gauche jusqu'à en haut à droite*/
      compt++;
      i++;
   }
   while((xmax-xmin)>compt);
   compt=0;
   do{
      image_write_pixel(picture,i,j,color);  /*on repart de haut-droite vers bas-droite*/
      compt++;
      j++;
   }
   while((ymax-ymin)>compt);
   compt=0;
   do{
      image_write_pixel(picture,i,j,color);  /*on repart de bas-droite vers bas-gauche*/
      compt++;
      i--;
   }
   while((xmax-xmin)>compt);
   compt=0;
   do{
      image_write_pixel(picture,i,j,color);  /*on repart de bas-gauche vers haut-gauche*/
      compt++;
      j--;
   }
   while((ymax-ymin)>compt);
